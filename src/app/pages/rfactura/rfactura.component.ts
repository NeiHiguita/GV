...
export default class RfacturaComponent {
  ...
  facturaSeleccionada: any = { id: null, fecha: '', tipo_proceso: '', subtotal: '', descripcion: '', estado: '' };
  ...
  editarFactura() {
    this.http.put(`http://localhost:3016/api/Factura/UpFactura/${this.facturaSeleccionada.id}`, this.facturaSeleccionada).subscribe(
      () => {
        this.obtenerFactura();
        this.cerrarModal();
  ...
  anularFactura(factura: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.http.put(`http://localhost:3016/api/Factura/EstadoFactura/${factura.id}`, { estado: nuevoEstado }).subscribe(
          () => {
