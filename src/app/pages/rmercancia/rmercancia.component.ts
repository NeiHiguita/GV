...
export default class RmercanciaComponent {
  ...
  mercanciaSeleccionada: any = { id: null, proveedor: '', producto: '', descripcion: '', precio: '', fecha: '', estado: '' };
  ...
  editarMercancia() {
    this.http.put(`http://localhost:3016/api/Mercancia/UpMercancia/${this.mercanciaSeleccionada.id}`, this.mercanciaSeleccionada).subscribe(
      () => {
        this.obtenerMercancia();
        this.cerrarModal();
  ...
  anularMercancia(mercancia: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.http.put(`http://localhost:3016/api/Mercancia/EstadoMercancia/${mercancia.id}`, { estado: nuevoEstado }).subscribe(
          () => {
