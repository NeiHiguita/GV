...
export default class ObservacionComponent implements OnInit {
  ...
  observacionSeleccionada: any = { id: null, observaciont: '', fecha: ''};
  ...
  editarObservacion() {
    this.http.put(`http://localhost:3016/api/Observacion/UpObservacion/${this.observacionSeleccionada.id}`, this.observacionSeleccionada).subscribe(
      () => {
        this.obtenerObservacion();
        this.cerrarModal();
  ...
  abrirModal() {
    ...
    this.observacionSeleccionada = { id: null, observacion: '', fecha: new Date().toISOString().slice(0, 16) }; 
  }
